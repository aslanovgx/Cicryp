import React from "react";
import "./../Popular/Popular.scss";
import { BiChevronDown } from "react-icons/bi";
import { IoCheckmarkDoneCircleSharp } from "react-icons/io5";

const Popular = () => {
  return (
    <section id="popular">
      <div className="container">
        <div className="row g-4">
          <div className="col-lg-12">
            <div className="popular_header">
              <h2>
                Popular
                <span>
                  {" "}
                  Creators
                  <BiChevronDown />
                </span>
              </h2>
            </div>
          </div>
          <div className="col-lg-3 col-md-6 col-sm-12">
            <div className="popular_card">
              <div className="popular_top">
                <img
                  src="https://images.unsplash.com/photo-1618172193622-ae2d025f4032?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDZ8fHxlbnwwfHx8fA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
                <img
                  src="https://images.unsplash.com/photo-1545898679-1d7a7701e2fb?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDEwfHx8ZW58MHx8fHw%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
                <img
                  src="https://images.unsplash.com/photo-1629946832022-c327f74956e0?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDd8fHxlbnwwfHx8fA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
              </div>
              <div className="popular_bottom">
                <div className="charlize_logo">
                  <img
                    src="https://chisnghiax.com/ciscryp/static/media/Image-4.36899b28c72dc4bc41a9.png"
                    alt=""
                  />
                </div>
                <div className="charlize">
                  <div className="charlize_left">
                    <h2>
                      Charlize Ray
                      <IoCheckmarkDoneCircleSharp />
                    </h2>
                    <span>@creator</span>
                  </div>
                  <ul className="charlize_right">
                    <li>
                      <a href="#">Follow</a>
                    </li>
                  </ul>
                </div>
                <div className="charlize_desc">
                  <p>
                    X-Metaverse is a Star Wars game based on NFT+ blockchain
                    exploration, mining, trading ...
                  </p>
                </div>
              </div>
            </div>
          </div>
          <div className="col-lg-3 col-md-6 col-sm-12">
            <div className="popular_card">
              <div className="popular_top">
                <img
                  src="https://images.unsplash.com/photo-1618172193622-ae2d025f4032?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDZ8fHxlbnwwfHx8fA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
                <img
                  src="https://images.unsplash.com/photo-1545898679-1d7a7701e2fb?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDEwfHx8ZW58MHx8fHw%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
                <img
                  src="https://images.unsplash.com/photo-1629946832022-c327f74956e0?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDd8fHxlbnwwfHx8fA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
              </div>
              <div className="popular_bottom">
                <div className="charlize_logo">
                  <img
                    src="https://chisnghiax.com/ciscryp/static/media/Image-4.36899b28c72dc4bc41a9.png"
                    alt=""
                  />
                </div>
                <div className="charlize">
                  <div className="charlize_left">
                    <h2>
                      Charlize Ray
                      <IoCheckmarkDoneCircleSharp />
                    </h2>
                    <span>@creator</span>
                  </div>
                  <ul className="charlize_right">
                    <li>
                      <a href="#">Follow</a>
                    </li>
                  </ul>
                </div>
                <div className="charlize_desc">
                  <p>
                    X-Metaverse is a Star Wars game based on NFT+ blockchain
                    exploration, mining, trading ...
                  </p>
                </div>
              </div>
            </div>
          </div>
          <div className="col-lg-3 col-md-6 col-sm-12">
            <div className="popular_card">
              <div className="popular_top">
                <img
                  src="https://images.unsplash.com/photo-1618172193622-ae2d025f4032?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDZ8fHxlbnwwfHx8fA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
                <img
                  src="https://images.unsplash.com/photo-1545898679-1d7a7701e2fb?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDEwfHx8ZW58MHx8fHw%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
                <img
                  src="https://images.unsplash.com/photo-1629946832022-c327f74956e0?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDd8fHxlbnwwfHx8fA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
              </div>
              <div className="popular_bottom">
                <div className="charlize_logo">
                  <img
                    src="https://chisnghiax.com/ciscryp/static/media/Image-4.36899b28c72dc4bc41a9.png"
                    alt=""
                  />
                </div>
                <div className="charlize">
                  <div className="charlize_left">
                    <h2>
                      Charlize Ray
                      <IoCheckmarkDoneCircleSharp />
                    </h2>
                    <span>@creator</span>
                  </div>
                  <ul className="charlize_right">
                    <li>
                      <a href="#">Follow</a>
                    </li>
                  </ul>
                </div>
                <div className="charlize_desc">
                  <p>
                    X-Metaverse is a Star Wars game based on NFT+ blockchain
                    exploration, mining, trading ...
                  </p>
                </div>
              </div>
            </div>
          </div>
          <div className="col-lg-3 col-md-6 col-sm-12">
            <div className="popular_card">
              <div className="popular_top">
                <img
                  src="https://images.unsplash.com/photo-1618172193622-ae2d025f4032?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDZ8fHxlbnwwfHx8fA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
                <img
                  src="https://images.unsplash.com/photo-1545898679-1d7a7701e2fb?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDEwfHx8ZW58MHx8fHw%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
                <img
                  src="https://images.unsplash.com/photo-1629946832022-c327f74956e0?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDd8fHxlbnwwfHx8fA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
              </div>
              <div className="popular_bottom">
                <div className="charlize_logo">
                  <img
                    src="https://chisnghiax.com/ciscryp/static/media/Image-4.36899b28c72dc4bc41a9.png"
                    alt=""
                  />
                </div>
                <div className="charlize">
                  <div className="charlize_left">
                    <h2>
                      Charlize Ray
                      <IoCheckmarkDoneCircleSharp />
                    </h2>
                    <span>@creator</span>
                  </div>
                  <ul className="charlize_right">
                    <li>
                      <a href="#">Follow</a>
                    </li>
                  </ul>
                </div>
                <div className="charlize_desc">
                  <p>
                    X-Metaverse is a Star Wars game based on NFT+ blockchain
                    exploration, mining, trading ...
                  </p>
                </div>
              </div>
            </div>
          </div>
          <div className="col-lg-3 col-md-6 col-sm-12">
            <div className="popular_card">
              <div className="popular_top">
                <img
                  src="https://images.unsplash.com/photo-1618172193622-ae2d025f4032?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDZ8fHxlbnwwfHx8fA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
                <img
                  src="https://images.unsplash.com/photo-1545898679-1d7a7701e2fb?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDEwfHx8ZW58MHx8fHw%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
                <img
                  src="https://images.unsplash.com/photo-1629946832022-c327f74956e0?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDd8fHxlbnwwfHx8fA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
              </div>
              <div className="popular_bottom">
                <div className="charlize_logo">
                  <img
                    src="https://chisnghiax.com/ciscryp/static/media/Image-4.36899b28c72dc4bc41a9.png"
                    alt=""
                  />
                </div>
                <div className="charlize">
                  <div className="charlize_left">
                    <h2>
                      Charlize Ray
                      <IoCheckmarkDoneCircleSharp />
                    </h2>
                    <span>@creator</span>
                  </div>
                  <ul className="charlize_right">
                    <li>
                      <a href="#">Follow</a>
                    </li>
                  </ul>
                </div>
                <div className="charlize_desc">
                  <p>
                    X-Metaverse is a Star Wars game based on NFT+ blockchain
                    exploration, mining, trading ...
                  </p>
                </div>
              </div>
            </div>
          </div>
          <div className="col-lg-3 col-md-6 col-sm-12">
            <div className="popular_card">
              <div className="popular_top">
                <img
                  src="https://images.unsplash.com/photo-1618172193622-ae2d025f4032?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDZ8fHxlbnwwfHx8fA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
                <img
                  src="https://images.unsplash.com/photo-1545898679-1d7a7701e2fb?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDEwfHx8ZW58MHx8fHw%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
                <img
                  src="https://images.unsplash.com/photo-1629946832022-c327f74956e0?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDd8fHxlbnwwfHx8fA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
              </div>
              <div className="popular_bottom">
                <div className="charlize_logo">
                  <img
                    src="https://chisnghiax.com/ciscryp/static/media/Image-4.36899b28c72dc4bc41a9.png"
                    alt=""
                  />
                </div>
                <div className="charlize">
                  <div className="charlize_left">
                    <h2>
                      Charlize Ray
                      <IoCheckmarkDoneCircleSharp />
                    </h2>
                    <span>@creator</span>
                  </div>
                  <ul className="charlize_right">
                    <li>
                      <a href="#">Follow</a>
                    </li>
                  </ul>
                </div>
                <div className="charlize_desc">
                  <p>
                    X-Metaverse is a Star Wars game based on NFT+ blockchain
                    exploration, mining, trading ...
                  </p>
                </div>
              </div>
            </div>
          </div>
          <div className="col-lg-3 col-md-6 col-sm-12">
            <div className="popular_card">
              <div className="popular_top">
                <img
                  src="https://images.unsplash.com/photo-1618172193622-ae2d025f4032?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDZ8fHxlbnwwfHx8fA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
                <img
                  src="https://images.unsplash.com/photo-1545898679-1d7a7701e2fb?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDEwfHx8ZW58MHx8fHw%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
                <img
                  src="https://images.unsplash.com/photo-1629946832022-c327f74956e0?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDd8fHxlbnwwfHx8fA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
              </div>
              <div className="popular_bottom">
                <div className="charlize_logo">
                  <img
                    src="https://chisnghiax.com/ciscryp/static/media/Image-4.36899b28c72dc4bc41a9.png"
                    alt=""
                  />
                </div>
                <div className="charlize">
                  <div className="charlize_left">
                    <h2>
                      Charlize Ray
                      <IoCheckmarkDoneCircleSharp />
                    </h2>
                    <span>@creator</span>
                  </div>
                  <ul className="charlize_right">
                    <li>
                      <a href="#">Follow</a>
                    </li>
                  </ul>
                </div>
                <div className="charlize_desc">
                  <p>
                    X-Metaverse is a Star Wars game based on NFT+ blockchain
                    exploration, mining, trading ...
                  </p>
                </div>
              </div>
            </div>
          </div>
          <div className="col-lg-3 col-md-6 col-sm-12">
            <div className="popular_card">
              <div className="popular_top">
                <img
                  src="https://images.unsplash.com/photo-1618172193622-ae2d025f4032?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDZ8fHxlbnwwfHx8fA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
                <img
                  src="https://images.unsplash.com/photo-1545898679-1d7a7701e2fb?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDEwfHx8ZW58MHx8fHw%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
                <img
                  src="https://images.unsplash.com/photo-1629946832022-c327f74956e0?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1yZWxhdGVkfDd8fHxlbnwwfHx8fA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt=""
                />
              </div>
              <div className="popular_bottom">
                <div className="charlize_logo">
                  <img
                    src="https://chisnghiax.com/ciscryp/static/media/Image-4.36899b28c72dc4bc41a9.png"
                    alt=""
                  />
                </div>
                <div className="charlize">
                  <div className="charlize_left">
                    <h2>
                      Charlize Ray
                      <IoCheckmarkDoneCircleSharp />
                    </h2>
                    <span>@creator</span>
                  </div>
                  <ul className="charlize_right">
                    <li>
                      <a href="#">Follow</a>
                    </li>
                  </ul>
                </div>
                <div className="charlize_desc">
                  <p>
                    X-Metaverse is a Star Wars game based on NFT+ blockchain
                    exploration, mining, trading ...
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Popular;
